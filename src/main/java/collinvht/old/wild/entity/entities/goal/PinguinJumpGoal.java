package collinvht.old.wild.entity.entities.goal;
...
public class PinguinJumpGoal extends JumpGoal {
    private static final int[] JUMP_DISTANCES = new int[]{0, 1, 4, 5, 6, 7};
    private final AnimalEntity dolphin;
    private final int field_220712_c;
    private boolean inWater;

    public PinguinJumpGoal(AnimalEntity dolphin, int p_i50329_2_) {
    ...
    public boolean shouldExecute() {
        if (this.dolphin.getRNG().nextInt(this.field_220712_c) != 0) {
            ...
            if(dolphin instanceof BluePinguinEntity) {
                BluePinguinEntity entity = (BluePinguinEntity) dolphin;
    ...
    public void startExecuting() {
        Direction direction = this.dolphin.getAdjustedHorizontalFacing();
        this.dolphin.setMotion(this.dolphin.getMotion().add((double)direction.getXOffset() * 0.8D, 0.6D, (double)direction.getZOffset() * 0.8D));
